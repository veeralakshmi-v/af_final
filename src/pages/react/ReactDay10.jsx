      ...
      {activeTab === 'state_lifting' && (
        <Section key="s6" eyebrow="Capstone Task • Day 10" title="Student Management API UI">
          <div className="panel" style={{ color: '#334155', lineHeight: 1.8 }}>
            ...
            {/* Pagination */}
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.6rem', marginBottom: '2.5rem' }}>
            ...
            <div style={{ borderTop: '2px dashed #e2e8f0', paddingTop: '2rem', marginTop: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: '0.5rem' }}>
                <span style={{ background: '#e0e7ff', color: '#4338ca', padding: '4px 10px', borderRadius: 6, fontSize: '0.78rem', fontWeight: 800, textTransform: 'uppercase' }}>
              ...
              <p style={{ fontSize: '0.92rem', color: '#475569', marginBottom: '1.5rem', lineHeight: 1.7 }}>
                This production-ready component combines <strong>all four REST API methods</strong> (<code>GET</code>, <code>POST</code>, <code>PUT</code>, <code>DELETE</code>), simulated network latencies, animated toast feedback, search filtering, controlled forms, and paginated data display.
              </p>

              {/* Architecture Highlights */}
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem', marginBottom: '1.5rem' }}>
                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '0.9rem' }}>
                  <div style={{ fontWeight: 800, color: '#0284c7', fontSize: '0.85rem', marginBottom: 4 }}>1. Async REST Handlers</div>
                  <div style={{ fontSize: '0.8rem', color: '#64748b' }}>Simulates async network delays with async/await and try/catch.</div>
                </div>
                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '0.9rem' }}>
                  <div style={{ fontWeight: 800, color: '#6366f1', fontSize: '0.85rem', marginBottom: 4 }}>2. Controlled Form State</div>
                  <div style={{ fontSize: '0.8rem', color: '#64748b' }}>Dual-mode form supporting both Create (POST) and Update (PUT).</div>
                </div>
                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '0.9rem' }}>
                  <div style={{ fontWeight: 800, color: '#10b981', fontSize: '0.85rem', marginBottom: 4 }}>3. Search & Pagination</div>
                  <div style={{ fontSize: '0.8rem', color: '#64748b' }}>Dynamic .filter() and .slice() math for client-side pagination.</div>
                </div>
                <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '0.9rem' }}>
                  <div style={{ fontWeight: 800, color: '#dc2626', fontSize: '0.85rem', marginBottom: 4 }}>4. Modal Confirmation</div>
                  <div style={{ fontSize: '0.8rem', color: '#64748b' }}>Overlay dialog protecting against accidental record deletions.</div>
                </div>
              </div>

              {/* Component CodeBlock */}
              <CodeBlock title="src/StudentManagementAPI.jsx" code={`import React, { useState, useEffect } from 'react';
...
export default function StudentManagementAPI() {
  ...
  const handleGetAll = async () => {
    setLoading(true);
    await sleep(700); // Simulate network request delay
    setLoading(false);
  ...
  // Auto-reset page to 1 when search query changes
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery]);
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ background: 'white', borderRadius: 16, padding: '2rem', maxWidth: 380, width: '90%', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center' }}>
              ...
              {/* Usage Guide */}
              <h4 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0f172a', marginTop: '2rem', marginBottom: '0.75rem' }}>
                🚀 How to Mount in <code>App.jsx</code>
              </h4>
              <CodeBlock title="src/App.jsx" code={`import React from 'react';
import StudentManagementAPI from './StudentManagementAPI';
