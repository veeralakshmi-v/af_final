import React, { useState } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Sparkles, ArrowRight, Layers, Play, Settings, Code, Clipboard, Terminal, FileJson, Brain, BookOpen, HelpCircle, Database, RotateCcw } from 'lucide-react';

const pageVariants = { 
  hidden: { opacity: 0, y: 15 }, 
  show: { opacity: 1, y: 0, transition: { staggerChildren: 0.07 } }, 
  exit: { opacity: 0, y: -10, transition: { duration: 0.15 } } 
};
const cardVariants = { hidden: { opacity: 0, y: 20 }, show: { opacity: 1, y: 0 } };

const TABS = [
  { id: 'intro', label: 'Overview', icon: <BookOpen size={15}/> },
  { id: 'architecture', label: 'Architecture', icon: <Brain size={15}/> },
  { id: 'practical', label: 'Practical Guide', icon: <Terminal size={15}/> },
  { id: 'sandbox', label: 'Sandbox', icon: <Play size={15}/> },
  { id: 'cheatsheet', label: 'Cheat Sheet', icon: <FileJson size={15}/> },
  { id: 'assignment', label: 'Assignment', icon: <Code size={15}/> },
  { id: 'quiz', label: 'Quiz', icon: <HelpCircle size={15}/> },
];

const STEPS = [
  { num: '01', title: 'Initialize Memory Checkpointer', icon: '💾', tag: 'Checkpointer Setup',
    body: 'Create a local in-memory saver memory context manager to record graph checkpoints at every state transaction step.',
    code: { 
      Terminal: '# Install checkpointer packages', 
      JSON: '{\n  "checkpointer": "MemorySaver"\n}', 
      Python: 'from langgraph.checkpoint.memory import MemorySaver\n\n# Instantiate memory manager checkpointer\nmemory = MemorySaver()',
      JavaScript: 'import { MemorySaver } from "@langchain/langgraph";\n\nconst memory = new MemorySaver();'
    } 
  },
  { num: '02', title: 'Compile Graph with Checkpointer', icon: '⚙️', tag: 'Graph Compilation',
    body: 'Pass the checkpointer instance directly to compiler parameters to enable state checkpoint updates saving automations.',
    code: { 
      Terminal: '# Compile graph with memory checkpoints', 
      JSON: '{\n  "compilation": {\n    "checkpointer": "MemorySaver"\n  }\n}', 
      Python: '# builder is compiled passing the memory saver checkpointer\napp = builder.compile(checkpointer=memory)',
      JavaScript: '// Compile in JS passing the memory checkpointer\nconst app = builder.compile({ checkpointer: memory });'
    } 
  },
  { num: '03', title: 'Execute Thread Configuration', icon: '🌐', tag: 'Execution Config',
    body: 'Run graph invocation pipelines supplying a unique thread_id parameter. This isolates conversations from other user channels.',
    code: { 
      Terminal: '# Invoke with thread configurations', 
      JSON: '{\n  "configurable": {\n    "thread_id": "user_session_445"\n  }\n}', 
      Python: '# Configure unique thread session identity\nconfig = {"configurable": {"thread_id": "thread_445"}}\n\napp.invoke({"messages": [("user", "Hello!")]}, config)',
      JavaScript: 'const config = { configurable: { threadId: "thread_445" } };\nawait app.invoke({ messages: [{ role: "user", content: "Hello!" }] }, config);'
    } 
  },
  { num: '04', title: 'Query and Inspect State History', icon: '🔍', tag: 'State Lookup',
    body: 'Query graph checkpoint history. Inspect exact variables or message listings recorded at previous node executions.',
    code: { 
      Terminal: '# No CLI dependencies', 
      JSON: '{\n  "history": "checkpoint_steps"\n}', 
      Python: '# Read current state variables matching configurations\nstate_info = app.get_state(config)\nprint("Current Count:", state_info.values.get("count"))\n\n# Print transaction list checkpoints history\nfor checkpoint in app.get_state_history(config):\n    print("Checkpoint step:", checkpoint.metadata.get("step"))',
      JavaScript: 'const stateInfo = await app.getState(config);\nconsole.log(stateInfo.values);\n\nfor await (const checkpoint of app.getStateHistory(config)) {\n  console.log(checkpoint.metadata.step);\n}'
    } 
  },
  { num: '05', title: 'Execute State Forking (Time Travel)', icon: '🔄', tag: 'State Forking',
    body: 'Update or modify state from previous history checkpoints to fork or redirect conversational agent branches.',
    code: { 
      Terminal: '# Execute time travel scripts', 
      JSON: '{\n  "forkState": {\n    "step": 2\n  }\n}', 
      Python: '# Update state variables targeting previous checkpoint values\nfork_config = {\n    "configurable": {\n        "thread_id": "thread_445",\n        "checkpoint_id": "1ef5-612a-3341"\n    }\n}\n\n# Updates states and forks active thread\napp.update_state(fork_config, {"count": 10})',
      JavaScript: 'const forkConfig = {\n  configurable: {\n    threadId: "thread_445",\n    checkpointId: "1ef5-612a-3341"\n  }\n};\n\nawait app.updateState(forkConfig, { count: 10 });'
    } 
  }
];

const QUIZ_QUESTIONS = [
  { q: 'What is a thread_id configuration used for in LangGraph?', opts: ['To speed up processing loops.', 'To isolate independent conversation history states across multiple users or sessions.', 'To clear cache.'], ans: 1 },
  { q: 'What is Time Travel in LangGraph?', opts: ['A visual page transition animation.', 'The ability to fetch historical checkpoints, inspect past states, and resume or fork execution from a past step.', 'A database speed optimization.'], ans: 1 },
  { q: 'Which class provides basic in-memory state persistence?', opts: ['SqliteSaver', 'MemorySaver', 'RedisSaver'], ans: 1 }
];

export default function AgenticAIDay29({ onNavigate, openAITutor }) {
  const [activeTab, setActiveTab] = useState('intro');
  const [activeStep, setActiveStep] = useState(0);
  const [codeTab, setCodeTab] = useState('Python');
  const [copied, setCopied] = useState(false);

  // Sandbox states
  const [userMsg, setUserMsg] = useState("");
  const [threadHistory, setThreadHistory] = useState([
    { checkpointId: 'cp_0', text: 'State Initialized', state: { count: 0, messages: [] } }
  ]);
  const [currentCheckpoint, setCurrentCheckpoint] = useState('cp_0');
  const [isRunning, setIsRunning] = useState(false);

  // Assignment / Quiz States
  const [assignmentText, setAssignmentText] = useState('');
  const [assignmentSubmitted, setAssignmentSubmitted] = useState(false);
  const [quizAnswers, setQuizAnswers] = useState({});
  const [quizSubmitted, setQuizSubmitted] = useState(false);

  const changeTab = (tabId) => { setActiveTab(tabId); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const copy = (text) => { navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 2000); };
  const quizScore = quizSubmitted ? Object.entries(quizAnswers).filter(([qi, ans]) => QUIZ_QUESTIONS[+qi]?.ans === ans).length : 0;

  const sendMessage = () => {
    if (!userMsg.trim()) return;
    setIsRunning(true);

    const activeCpObj = threadHistory.find(h => h.checkpointId === currentCheckpoint) || threadHistory[threadHistory.length - 1];
    const newCount = activeCpObj.state.count + 1;
    const newMessages = [...activeCpObj.state.messages, { sender: 'user', text: userMsg }];

    setTimeout(() => {
      const botReply = `Processed message ${newCount}: "${userMsg}".`;
      const finalizedMessages = [...newMessages, { sender: 'agent', text: botReply }];
      
      const newCpId = `cp_${Date.now()}`;
      const newCheckpoint = {
        checkpointId: newCpId,
        text: `Turn ${newCount}: ${userMsg}`,
        state: { count: newCount, messages: finalizedMessages }
      };

      // Fork history from current checkpoint point
      const activeCpIdx = threadHistory.findIndex(h => h.checkpointId === currentCheckpoint);
      const newHistory = [...threadHistory.slice(0, activeCpIdx + 1), newCheckpoint];

      setThreadHistory(newHistory);
      setCurrentCheckpoint(newCpId);
      setUserMsg("");
      setIsRunning(false);
    }, 1000);
  };

  const activeCheckpointObj = threadHistory.find(h => h.checkpointId === currentCheckpoint) || threadHistory[threadHistory.length - 1];

  return (
    <div style={{ maxWidth: '1080px', margin: '0 auto', width: '100%', paddingBottom: '5rem', fontFamily: "'Inter', sans-serif" }}>
      {/* Tab Nav */}
      <div style={{ display: 'flex', gap: '0.3rem', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 16, padding: '0.4rem', marginBottom: '2.5rem', overflowX: 'auto' }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => changeTab(t.id)}
            style={{ display: 'flex', alignItems: 'center', gap: '6px', background: activeTab === t.id ? 'white' : 'transparent', color: activeTab === t.id ? '#0284c7' : '#64748b', border: 'none', padding: '0.55rem 1rem', borderRadius: 12, cursor: 'pointer', fontWeight: 700, fontSize: '0.88rem', whiteSpace: 'nowrap', transition: 'all .15s', boxShadow: activeTab === t.id ? '0 2px 8px rgba(0,0,0,.08)' : 'none' }}>
            {t.icon}{t.label}
          </button>
        ))}
      </div>

      <AnimatePresence mode="wait">
        {/* OVERVIEW */}
        {activeTab === 'intro' && (
          <motion.div key="intro" variants={pageVariants} initial="hidden" animate="show" exit="exit">
            <motion.div variants={cardVariants} style={{ background: 'linear-gradient(135deg, #0284c7 0%, #0369a1 100%)', borderRadius: 28, padding: '3rem', color: 'white', marginBottom: '2.5rem', position: 'relative', overflow: 'hidden' }}>
              <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at 80% 50%, rgba(255,255,255,.06), transparent 60%)' }} />
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.25)', padding: '0.4rem 1rem', borderRadius: '30px', fontSize: '0.85rem', fontWeight: 700, color: '#e0f2fe', marginBottom: '1.2rem' }}>
                <Sparkles size={14} color="#fef08a" /> MODULE 6 • DAY 29
              </span>
              <h1 style={{ fontSize: '2.4rem', fontWeight: 800, margin: '0 0 1rem 0' }}>Persistence & Memory</h1>
              <p style={{ color: '#e0f2fe', fontSize: '1.1rem', lineHeight: 1.7, margin: 0 }}>
                Learn checkpointers and thread isolation. Explore how persistence layers store graph checkpoints and enable Time Travel state restoration.
              </p>
            </motion.div>

            <motion.div variants={cardVariants} style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '2.5rem', marginBottom: '2.5rem' }}>
              <div>
                <h3 style={{ fontSize: '1.4rem', fontWeight: 800, color: '#0f172a', marginBottom: '1rem' }}>Thread Checkpoints</h3>
                <p style={{ color: '#475569', fontSize: '1.02rem', lineHeight: 1.75, marginBottom: '1rem' }}>
                  LangGraph introduces first-class persistence via **Checkpointers**. As the graph runs, its state is written to storage at the end of each node transaction.
                </p>
                <p style={{ color: '#475569', fontSize: '1.02rem', lineHeight: 1.75, marginBottom: '1.5rem' }}>
                  This guarantees fault tolerance: if the pipeline crashes or requires human intervention middle-execution, it can resume from the last saved checkpoint. Isolation is managed by supplying a unique `thread_id`.
                </p>
              </div>
              <div style={{ background: '#f0f9ff', border: '1px solid #e0f2fe', borderRadius: 20, padding: '1.8rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <h4 style={{ fontSize: '1.15rem', color: '#0369a1', fontWeight: 800, margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <RotateCcw size={18} /> Time Travel
                </h4>
                <p style={{ color: '#475569', fontSize: '0.92rem', lineHeight: 1.6, margin: 0 }}>
                  Because states are versioned at each checkpoint step, you can query checkpoint history. Time Travel allows users or developers to:
                </p>
                <ul style={{ fontSize: '0.85rem', color: '#64748b', paddingLeft: 16, margin: 0 }}>
                  <li>View exact state variables at Step 2.</li>
                  <li>Fork execution back to Step 1 to try a different query path.</li>
                </ul>
              </div>
            </motion.div>

            <motion.div variants={cardVariants} style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button className="btn btn-primary" onClick={() => changeTab('architecture')} style={{ background: '#0284c7', borderColor: '#0284c7' }}>
                Explore Persistence Architecture <ArrowRight size={18} />
              </button>
            </motion.div>
          </motion.div>
        )}

        {/* ARCHITECTURE */}
        {activeTab === 'architecture' && (
          <motion.div key="architecture" variants={pageVariants} initial="hidden" animate="show" exit="exit">
            <motion.div variants={cardVariants} style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: 24, padding: '2.5rem', marginBottom: '2.5rem' }}>
              <h2 style={{ fontSize: '1.6rem', fontWeight: 800, color: '#0f172a', marginBottom: '0.5rem' }}>🏗️ Persistence checkpointing Lifecycle</h2>
              <p style={{ color: '#64748b', marginBottom: '1.8rem', fontSize: '1rem' }}>Checkpointers automatically save states after every node execution step:</p>
              <div style={{ background: '#0f172a', padding: '2.5rem', borderRadius: 16, border: '1px solid #1e293b', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px' }}>
                
                <div style={{ display: 'flex', gap: '12px', alignItems: 'center', color: 'white', background: '#1e293b', padding: '1rem 2rem', borderRadius: 12 }}>
                  <div style={{ background: '#0284c7', padding: '6px 12px', borderRadius: 6, fontWeight: 700, fontSize: '.85rem' }}>Node 1 runs</div>
                  <div>➔</div>
                  <div style={{ border: '1px solid #10b981', background: 'rgba(16,185,129,0.05)', padding: '6px 12px', borderRadius: 6, fontWeight: 700, fontSize: '.85rem' }}>Checkpointer saves checkpoint 1</div>
                  <div>➔</div>
                  <div style={{ background: '#0284c7', padding: '6px 12px', borderRadius: 6, fontWeight: 700, fontSize: '.85rem' }}>Node 2 runs</div>
                  <div>➔</div>
                  <div style={{ border: '1px solid #10b981', background: 'rgba(16,185,129,0.05)', padding: '6px 12px', borderRadius: 6, fontWeight: 700, fontSize: '.85rem' }}>Checkpointer saves checkpoint 2</div>
                </div>

                <p style={{ color: '#94a3b8', fontSize: '0.88rem', margin: '10px 0 0 0', maxWidth: '600px' }}>
                  If a thread crash occurs during Node 2 execution, LangGraph reads checkpoint 1 from the sqlite/memory checkpointer database and restarts the process with zero data losses.
                </p>
              </div>
            </motion.div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <button className="btn btn-outline" onClick={() => changeTab('intro')}>← Back</button>
              <button className="btn btn-primary" onClick={() => changeTab('practical')} style={{ background: '#0284c7', borderColor: '#0284c7' }}>View Practical Guide <ArrowRight size={18}/></button>
            </div>
          </motion.div>
        )}

        {/* PRACTICAL GUIDE */}
        {activeTab === 'practical' && (
          <motion.div key="practical" variants={pageVariants} initial="hidden" animate="show" exit="exit">
            <motion.div variants={cardVariants}>
              <h2 style={{ fontSize: '1.7rem', fontWeight: 800, color: '#0f172a', marginBottom: '.4rem' }}>🛠️ Step-by-Step Graph Building</h2>
              <p style={{ color: '#64748b', marginBottom: '2rem' }}>Define checkpointers, query thread configs, inspect variables, and fork state history:</p>
            </motion.div>

            <motion.div variants={cardVariants} style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: '1.5rem', alignItems: 'stretch', marginBottom: '2.5rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                {STEPS.map((s, i) => (
                  <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                    <button onClick={() => setActiveStep(i)}
                      style={{ width: '100%', display: 'flex', alignItems: 'center', gap: '0.9rem', background: activeStep === i ? '#0284c7' : 'white', border: activeStep === i ? '1px solid #0284c7' : '1px solid #e2e8f0', color: activeStep === i ? 'white' : '#1e293b', padding: '0.9rem 1rem', borderRadius: 14, cursor: 'pointer', textAlign: 'left', transition: 'all .2s' }}>
                      <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: 36, height: 36, borderRadius: 10, background: activeStep === i ? 'rgba(255,255,255,.2)' : '#f1f5f9', fontSize: '1rem', flexShrink: 0 }}>{s.icon}</span>
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontWeight: 800, fontSize: '.75rem', opacity: .65, marginBottom: 1 }}>Step {s.num}</div>
                        <div style={{ fontWeight: 700, fontSize: '.88rem', lineHeight: 1.3 }}>{s.title}</div>
                      </div>
                    </button>
                    {i < STEPS.length - 1 && (
                      <div style={{ width: 2, height: 10, background: i < activeStep ? '#0284c7' : '#e2e8f0', margin: '0 0 0 26px', transition: 'background .3s' }}/>
                    )}
                  </div>
                ))}
              </div>

              <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 20, overflow: 'hidden', display: 'flex', flexDirection: 'column', minHeight: 400 }}>
                <div style={{ padding: '1.4rem 1.8rem', borderBottom: '1px solid #1e293b', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  <span style={{ fontSize: '1.8rem' }}>{STEPS[activeStep].icon}</span>
                  <div>
                    <div style={{ fontSize: '.7rem', color: '#0284c7', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 2 }}>{STEPS[activeStep].tag}</div>
                    <div style={{ fontWeight: 800, fontSize: '1.1rem', color: 'white' }}>{STEPS[activeStep].title}</div>
                  </div>
                </div>
                <div style={{ padding: '1.2rem 1.8rem', borderBottom: '1px solid #1e293b' }}>
                  <p style={{ color: '#94a3b8', fontSize: '.92rem', lineHeight: 1.7, margin: 0 }}>{STEPS[activeStep].body}</p>
                </div>
                <div style={{ display: 'flex', gap: '.3rem', padding: '.75rem 1.8rem .4rem', borderBottom: '1px solid #1e293b' }}>
                  {['JSON','Python','JavaScript'].map(t => (
                    <button key={t} onClick={() => setCodeTab(t)} style={{ display: 'flex', alignItems: 'center', gap: 5, background: codeTab === t ? '#0284c7' : 'transparent', color: codeTab === t ? 'white' : '#64748b', border: codeTab === t ? 'none' : '1px solid #334155', padding: '.35rem .8rem', borderRadius: 8, cursor: 'pointer', fontWeight: 700, fontSize: '.78rem' }}>
                      {t}
                    </button>
                  ))}
                  <div style={{ flex: 1 }}/>
                  <button onClick={() => copy(STEPS[activeStep].code[codeTab])} style={{ display: 'flex', alignItems: 'center', gap: 5, background: copied ? '#059669' : '#1e293b', color: 'white', border: 'none', padding: '.35rem .8rem', borderRadius: 8, cursor: 'pointer', fontSize: '.75rem', fontWeight: 700 }}>
                    {copied ? 'Copied!' : 'Copy'}
                  </button>
                </div>
                <div style={{ flex: 1, padding: '1.2rem 1.8rem', overflowY: 'auto' }}>
                  <pre style={{ margin: 0, fontSize: '.82rem', fontFamily: 'monospace', lineHeight: 1.8, color: codeTab === 'JSON' ? '#fbbf24' : '#60a5fa', whiteSpace: 'pre-wrap' }}>{STEPS[activeStep].code[codeTab]}</pre>
                </div>
              </div>
            </motion.div>

            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <button className="btn btn-outline" onClick={() => changeTab('architecture')}>← Back</button>
              <button className="btn btn-primary" onClick={() => changeTab('sandbox')} style={{ background: '#0284c7', borderColor: '#0284c7' }}>Try Sandbox <ArrowRight size={18}/></button>
            </div>
          </motion.div>
        )}

        {/* SANDBOX */}
        {activeTab === 'sandbox' && (
          <motion.div key="sandbox" variants={pageVariants} initial="hidden" animate="show" exit="exit">
            <motion.div variants={cardVariants}>
              <h2 style={{ fontSize: '1.7rem', fontWeight: 800, color: '#0f172a', marginBottom: '.4rem' }}>💻 Checkpoint Memory & Time Travel</h2>
              <p style={{ color: '#64748b', marginBottom: '2rem' }}>Send messages to build state checkpoints. Click any past turn to restore/travel back in time:</p>
            </motion.div>

            <motion.div variants={cardVariants} style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: '1.5rem', marginBottom: '2.5rem' }}>
              <div style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: 20, padding: '1.8rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <h3 style={{ fontSize: '1rem', fontWeight: 800, color: '#0f172a', margin: 0 }}>💬 Thread Chat Input</h3>
                
                <div style={{ display: 'flex', gap: '8px' }}>
                  <input type="text" value={userMsg} onChange={e => setUserMsg(e.target.value)} disabled={isRunning} placeholder="Type: 'Task A complete'..." style={{ flex: 1, padding: '.6rem', border: '1px solid #e2e8f0', borderRadius: 8, fontSize: '.84rem', outline: 'none' }}/>
                  <button onClick={sendMessage} disabled={isRunning || !userMsg.trim()} style={{ background: '#0284c7', color: 'white', border: 'none', padding: '0 .9rem', borderRadius: 8, fontWeight: 700, cursor: 'pointer' }}>
                    Send
                  </button>
                </div>

                {/* Checkpoint list */}
                <div style={{ marginTop: '0.5rem' }}>
                  <label style={{ fontSize: '.75rem', color: '#94a3b8', display: 'block', marginBottom: 6, fontWeight: 700 }}>⏳ Saved Checkpoints (Thread ID: session_101):</label>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 180, overflowY: 'auto' }}>
                    {threadHistory.map((h) => {
                      const isActive = h.checkpointId === currentCheckpoint;
                      return (
                        <button key={h.checkpointId} onClick={() => setCurrentCheckpoint(h.checkpointId)}
                          style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', borderRadius: 8, background: isActive ? '#f0f9ff' : 'white', border: isActive ? '1px solid #0284c7' : '1px solid #e2e8f0', cursor: 'pointer', textAlign: 'left' }}>
                          <span style={{ fontSize: '.78rem', fontWeight: isActive ? 700 : 500, color: isActive ? '#0369a1' : '#475569' }}>{h.text}</span>
                          <span style={{ fontSize: '.65rem', color: isActive ? '#10b981' : '#94a3b8', background: isActive ? '#ecfdf5' : '#f1f5f9', padding: '2px 6px', borderRadius: 4 }}>
                            {isActive ? 'Active Checkpoint' : 'Time Travel'}
                          </span>
                        </button>
                      );
                    })}
                  </div>
                </div>
              </div>

              <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 20, padding: '1.8rem', display: 'flex', flexDirection: 'column', gap: '1rem', minHeight: 340 }}>
                <div style={{ flex: 1, overflowY: 'auto', background: '#1e293b', padding: '1.2rem', borderRadius: 12, border: '1px solid #334155', display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                  <span style={{ display: 'block', fontSize: '.65rem', color: '#94a3b8', fontFamily: 'monospace', textTransform: 'uppercase', borderBottom: '1px solid #334155', paddingBottom: '.3rem', marginBottom: '.4rem' }}>
                    💾 Restored State Variables at: {currentCheckpoint}
                  </span>

                  <div style={{ fontSize: '0.8rem', fontFamily: 'monospace', color: '#38bdf8', marginBottom: 8 }}>
                    State.count = {activeCheckpointObj?.state.count}
                  </div>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.50rem' }}>
                    {activeCheckpointObj?.state.messages.length === 0 ? (
                      <span style={{ fontSize: '.78rem', color: '#64748b', fontStyle: 'italic' }}>No messages recorded at this checkpoint.</span>
                    ) : (
                      activeCheckpointObj.state.messages.map((m, i) => (
                        <div style={{ 
                          fontSize: '.84rem', 
                          padding: '0.5rem 0.7rem', 
                          borderRadius: 8,
                          background: m.sender === 'user' ? 'rgba(255,255,255,0.03)' : 'rgba(2,132,199,0.08)',
                          color: m.sender === 'user' ? '#e2e8f0' : '#38bdf8', 
                          border: '1px solid #334155'
                        }} key={i}>
                          <strong>{m.sender === 'user' ? '👤 User' : '🤖 Agent'}:</strong> {m.text}
                        </div>
                      ))
                    )}
                  </div>
                </div>
              </div>
            </motion.div>

            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <button className="btn btn-outline" onClick={() => changeTab('practical')}>← Back</button>
              <button className="btn btn-primary" onClick={() => changeTab('cheatsheet')} style={{ background: '#0284c7', borderColor: '#0284c7' }}>View Cheat Sheet <ArrowRight size={18}/></button>
            </div>
          </motion.div>
        )}

        {/* CHEAT SHEET */}
        {activeTab === 'cheatsheet' && (
          <motion.div key="cheatsheet" variants={pageVariants} initial="hidden" animate="show" exit="exit">
            <motion.div variants={cardVariants} style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: 20, padding: '2.5rem', marginBottom: '2.5rem' }}>
              <h2 style={{ fontSize: '1.6rem', fontWeight: 800, color: '#0f172a', marginBottom: '0.5rem' }}>📋 Day 29 Cheat Sheet</h2>
              <p style={{ color: '#64748b', marginBottom: '2rem' }}>Core classes and state retrieval queries references:</p>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem', marginBottom: '2rem' }}>
                <div>
                  <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0f172a', marginBottom: '1rem', borderBottom: '2px solid #0284c7', paddingBottom: '0.4rem' }}>🐍 Python Reference</h3>
                  <pre style={{ background: '#0f172a', color: '#60a5fa', padding: '1.2rem', borderRadius: 12, fontSize: '0.78rem', lineHeight: 1.6, overflowX: 'auto', fontFamily: 'monospace' }}>
{`# 1. Compile with checkpointer
from langgraph.checkpoint.memory import MemorySaver

memory = MemorySaver()
app = builder.compile(checkpointer=memory)

# 2. Query configurations
config = {"configurable": {"thread_id": "session_id"}}

# 3. Read Checkpoints
state_info = app.get_state(config)
checkpoint_history = list(app.get_state_history(config))`}
                  </pre>
                </div>

                <div>
                  <h3 style={{ fontSize: '1.1rem', fontWeight: 800, color: '#0f172a', marginBottom: '1rem', borderBottom: '2px solid #0284c7', paddingBottom: '0.4rem' }}>🟨 JavaScript Reference</h3>
                  <pre style={{ background: '#0f172a', color: '#60a5fa', padding: '1.2rem', borderRadius: 12, fontSize: '0.78rem', lineHeight: 1.6, overflowX: 'auto', fontFamily: 'monospace' }}>
{`// 1. Compile with checkpointer
import { MemorySaver } from "@langchain/langgraph";

const memory = new MemorySaver();
const app = builder.compile({ checkpointer: memory });

// 2. Query configurations
const config = { configurable: { threadId: "session_id" } };

// 3. Read Checkpoints in JS
const stateInfo = await app.getState(config);
const history = [];
for await (const checkpoint of app.getStateHistory(config)) {
  history.push(checkpoint);
}`}
                  </pre>
                </div>
              </div>
            </motion.div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <button className="btn btn-outline" onClick={() => changeTab('sandbox')}>← Back</button>
              <button className="btn btn-primary" onClick={() => changeTab('assignment')} style={{ background: '#0284c7', borderColor: '#0284c7' }}>Go to Assignment <ArrowRight size={18}/></button>
            </div>
          </motion.div>
        )}

        {/* ASSIGNMENT */}
        {activeTab === 'assignment' && (
          <motion.div key="assignment" variants={pageVariants} initial="hidden" animate="show" exit="exit">
            <motion.div variants={cardVariants} style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: 20, padding: '2.5rem', marginBottom: '2rem' }}>
              <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#0f172a', marginBottom: '1rem' }}>📝 Day 29 Assignment: Setup SQLite Checkpointer</h2>
              <p style={{ color: '#475569', lineHeight: 1.6, marginBottom: '1.5rem' }}>Write a Python program that sets up a SqliteSaver checkpointer. Compile the graph with it and run a search workflow using thread_id=102. Show state is retrieved from the database.</p>
              <textarea value={assignmentText} onChange={e => setAssignmentText(e.target.value)} placeholder="from langgraph.checkpoint.sqlite import SqliteSaver&#10;...&#10;with SqliteSaver.from_conn_string(':memory:') as memory:&#10;    app = builder.compile(checkpointer=memory)&#10;..." style={{ width: '100%', height: 180, padding: '1rem', borderRadius: 12, border: '1px solid #e2e8f0', fontSize: '.92rem', outline: 'none', resize: 'none', boxSizing: 'border-box', fontFamily: 'monospace' }}/>
              <button onClick={() => setAssignmentSubmitted(true)} disabled={!assignmentText.trim() || assignmentSubmitted} style={{ background: '#0284c7', color: 'white', border: 'none', padding: '.75rem 1.5rem', borderRadius: 10, fontWeight: 700, cursor: 'pointer', marginTop: '1rem' }}>
                {assignmentSubmitted ? '✅ Submitted!' : 'Submit Code'}
              </button>
            </motion.div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <button className="btn btn-outline" onClick={() => changeTab('cheatsheet')}>← Back</button>
              <button className="btn btn-primary" onClick={() => changeTab('quiz')} style={{ background: '#0284c7', borderColor: '#0284c7' }}>Take Quiz <ArrowRight size={18}/></button>
            </div>
          </motion.div>
        )}

        {/* QUIZ */}
        {activeTab === 'quiz' && (
          <motion.div key="quiz" variants={pageVariants} initial="hidden" animate="show" exit="exit">
            <motion.div variants={cardVariants} style={{ background: 'white', border: '1px solid #e2e8f0', borderRadius: 20, padding: '2.5rem', marginBottom: '2rem' }}>
              <h2 style={{ fontSize: '1.6rem', fontWeight: 800, color: '#0f172a', marginBottom: '1.5rem' }}>✍️ Day 29 Quiz Assessment</h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                {QUIZ_QUESTIONS.map((q, qi) => {
                  const sel = quizAnswers[qi];
                  return (
                    <div key={qi} style={{ borderBottom: '1px solid #f1f5f9', paddingBottom: '1.5rem' }}>
                      <strong style={{ fontSize: '.97rem', color: '#0f172a', display: 'block', marginBottom: '0.8rem' }}>Q{qi+1}: {q.q}</strong>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '.5rem' }}>
                        {q.opts.map((opt, oi) => {
                          let bg = '#f8fafc', border = '1px solid #e2e8f0', color = '#475569';
                          if (quizSubmitted) { if (oi === q.ans) { bg = '#ecfdf5'; border = '1px solid #10b981'; color = '#166534'; } else if (sel === oi) { bg = '#fef2f2'; border = '1px solid #ef4444'; color = '#991b1b'; } }
                          else if (sel === oi) { bg = '#e0f2fe'; border = '1px solid #0284c7'; color = '#0284c7'; }
                          return <div key={oi} onClick={() => !quizSubmitted && setQuizAnswers(p => ({ ...p, [qi]: oi }))} style={{ bg, border, color, padding: '.8rem 1.1rem', borderRadius: 10, cursor: quizSubmitted ? 'default' : 'pointer', fontSize: '.92rem', transition: 'all .15s' }}>{opt}</div>;
                        })}
                      </div>
                    </div>
                  );
                })}
              </div>
              {!quizSubmitted ? (
                <button onClick={() => setQuizSubmitted(true)} disabled={Object.keys(quizAnswers).length < QUIZ_QUESTIONS.length} style={{ background: '#0284c7', color: 'white', border: 'none', padding: '.85rem 2rem', borderRadius: 10, fontWeight: 700, cursor: 'pointer', marginTop: '2rem' }}>Submit Answers</button>
              ) : (
                <div style={{ marginTop: '2rem', background: 'linear-gradient(135deg,#f0f9ff,#e0f2fe)', border: '1px solid #bae6fd', borderRadius: 16, padding: '1.5rem', textAlign: 'center' }}>
                  <strong style={{ fontSize: '1.3rem', color: '#0369a1', display: 'block', marginBottom: '.3rem' }}>Score: {quizScore} / {QUIZ_QUESTIONS.length}</strong>
                  <span style={{ color: '#0284c7' }}>{quizScore === QUIZ_QUESTIONS.length ? '⭐ Perfect score!' : 'Review the highlighted correct answers.'}</span>
                </div>
              )}
            </motion.div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <button className="btn btn-outline" onClick={() => changeTab('assignment')}>← Back</button>
              <button className="btn btn-primary" onClick={() => onNavigate('agentic_ai_module6', 'day30')} style={{ background: '#0f172a', color: 'white', border: 'none', padding: '.8rem 1.6rem', borderRadius: 12, fontWeight: 700 }}>Next Day (Day 30) →</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
  );
}
